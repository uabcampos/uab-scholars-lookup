...
v0.1.5 (2024-03-24)
- Resolved reserved key conflicts by renaming "type" to "result_type" to prevent runtime errors in LLM environments
- Enhanced JSON schema for safer and more consistent responses
- Implemented internal method error guarding to ensure returned values are of expected types
- Made event emission fully optional and safe to avoid runtime errors
- Improved async/await hygiene to reduce concurrency overhead
- Enhanced error reporting and debug output for better user experience
                ...
                for scholar in data.get("resource", []):
                    ...
                    if any(department.lower() in (p.get("department","") or "").lower() for p in positions):
                        ...
                        if disc_id:
                            ...
                            scholar_data = {
                                "object": "department_search_results",
                                "status": "found",
                                "message": f"Found scholar: {name}",
                                "profile": {
                                    "name": name,
